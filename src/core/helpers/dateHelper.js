function formatDate(date, format) {
  ...
  date = typeof date != "undefined" && date instanceof Date ? date : new Date(date);
  ...
  var formats = {
    ...
    T: function (date) {
      ...
      if (abbreviation) abbreviation = abbreviation[1].match(/[A-Z]/g).join("");
      return abbreviation;
    },
    u: function (date) {
  ...
  for (var position = 0; position < format.length; position++) {
    if (!escaped && format.substring(position, position + 1) == "\\") escaped = true;
      ...
      escaped = false;
    } else string += String(formats[format.substring(position, position + 1)](date));
  }
  return string;
}
